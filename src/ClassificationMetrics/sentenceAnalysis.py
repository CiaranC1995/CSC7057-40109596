...
def display_individual_sentence_stats(input_text):
    ...
    for s in sentences:
        print(s)
        print(f'Number of words in sentence : {num_total_words(s)}')
...
def display_stats_for_whole_text(input_text):
    ...
    min_words = min(sentence_lengths(input_text))
    max_words = max(sentence_lengths(input_text))

    print(f'Number of Words in text : {num_total_words(input_text)}')
    print(f'Average number of words per sentence : {round(average_sentence_length_words(input_text), 3)}')
    print(f'Average number of chars per sentence : {round(average_sentence_length_chars(input_text), 3)}')
...
def plot_sentence_length_distribution(input_text):
    ...
    lengths = sentence_lengths(input_text)
...
def calculate_flesch_kincaid(input_text):
    ...
    num_words = num_total_words(input_text)
    processed_input_text = pp.TextPreprocessor.remove_punctuation(input_text)
    words = nltk.word_tokenize(processed_input_text)
