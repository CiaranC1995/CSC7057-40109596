...
final_text = dataset['text_to_analyse'].head(65000).to_list()
final_text_vectorized = vectorizer.transform(final_text)
final_ppl = dataset['perplexity'].head(65000).to_list()
final_burstiness = dataset['burstiness'].head(65000).to_list()
final_labels = dataset['AI Generated'].head(65000).to_list()

print('Number of Total Records :', len(dataset))
print('Number of Training Records :', len(dataset) * 0.8)
print('Number of Validation Records :', len(dataset) * 0.2)
...
for iteration_count, i in enumerate(range(num_iterations)):
    ...
    hstack_batch_size = 10000  # Choose an appropriate batch size
...
# Create separate box plots for AI generated and human written prediction probabilities
plt.figure(figsize=(10, 6))
plt.subplot(1, 2, 1)
plt.title('AI Generated Prediction Probabilities')
sns.boxplot(data=[svm_ai_prob], orient='v', showfliers=True)
plt.ylabel('Probability')
plt.tight_layout()

plt.subplot(1, 2, 2)
plt.title('Human Written Prediction Probabilities')
sns.boxplot(data=[svm_human_prob], orient='v', showfliers=True)
plt.ylabel('Probability')
plt.tight_layout()
